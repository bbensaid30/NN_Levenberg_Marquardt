...
Sdouble const& alpha, Sdouble const& pas, Sdouble const& eps, int const& maxIter, Sdouble const& epsNeight)
{
    ...
    for(i=0;i<nbTirage;i++)
    {
        ...
        study = train(X,Y,L,nbNeurons,globalIndices,activations,weights,bias,algo,eps,maxIter,mu,factor,RMin,RMax,b,alpha,pas,Rlim,factorMin,power,alphaChap,epsDiag,0.1,2.0,3.0,3,0.1,"2",1);
...
void testPolyFour(std::string const& distribution, std::vector<double> const& supParameters, int const& nbTirage, std::string const& algo, Sdouble& mu, Sdouble& factor, Sdouble const& Rlim,
...
Sdouble const& alpha, Sdouble const& pas, Sdouble const& eps, int const& maxIter, Sdouble const& epsNeight)
{
    ...
    {
        seed=i; initialisation(nbNeurons,weights,bias,supParameters,distribution,seed);
        study = train_entropie(X,Y,L,nbNeurons,globalIndices,activations,weights,bias,algo,eps,maxIter,mu,factor,RMin,RMax,b,epsDiag,0.1,2.0,3.0,3,0.1);
