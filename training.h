...
double mu=10, double factor=10, double const eps=std::pow(10,-7), int const maxIter=2000,bool const record=false, std::string const fileExtension="");
...
std::vector<std::string> const& activations, std::vector<Eigen::MatrixXd>& weights, std::vector<Eigen::VectorXd>& bias,
double const eps=std::pow(10,-7), int const maxIter=2000, double const sigma=0.1, bool const recocrd=false, std::string const fileExtension="");
