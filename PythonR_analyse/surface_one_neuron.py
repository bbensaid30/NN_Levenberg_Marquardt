...
def dataSquareWave(P,frequence=1):
    X=np.linspace(-1,1,num=P)
    Y=2*(2*np.floor(frequence*X)-np.floor(2*frequence*X))+1
    return X,Y

def dataSquareRoot(P):
    X=np.linspace(0,2,num=P)
    Y=np.sqrt(X)
    return X,Y
...
def surfaceNetwork(g,data,P,ex1,ex2,nbPoints, normer=False, plotPoints=False, fileNameCost="", fileNameŴeight=""):
    ...
    if data=="SineWave":
        ...
        X,Y=dataSquare(P)
    elif data=="SquareWave":
        X,Y=dataSquareWave(P)
    elif data=="SquareRoot":
        X,Y=dataSquareRoot(P)
    elif data=="Exp":
        X,Y=dataExp(P)
    ...
    ax.set_xlim(ex1,ex2); ax.set_ylim(ex1,ex2); ax.zaxis.set_major_locator(LinearLocator(10)); ax.zaxis.set_major_formatter('{x:.02f}')
...
surfaceNetwork(sigmoid,"Sinc1",40,-15,15,100,False,False,fileNameCost,fileNameWeight)