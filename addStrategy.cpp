...
std::vector<std::vector<Eigen::VectorXd>>& biasList, double const& currentCost, double const& currentGradientNorm, std::vector<double>& costs, std::vector<double>& gradientNorms,
Eigen::MatrixXd const& X, Eigen::MatrixXd const& Y, int const& L, int const& P, std::vector<int> const& nbNeurons, std::vector<int> const& globalIndices, std::vector<std::string> const& activations,
double const epsClose, int const nbDichotomie, double const eRelative, std::string const strategy)
{
    if (strategy=="CostCloser")
    {
        return addCostCloser(weights,bias,weightsList,biasList,currentCost,currentGradientNorm,costs,gradientNorms,X,Y,L,P,nbNeurons,globalIndices,activations,epsClose,nbDichotomie,eRelative);
    }
    else if(strategy=="CostAll")
    {
        return addCostAll(weights,bias,weightsList,biasList,currentCost,currentGradientNorm,costs,gradientNorms,X,Y,L,P,nbNeurons,globalIndices,activations,epsClose,nbDichotomie,eRelative);
    }
    else if(strategy=="CostAllSet")
    {
        return addCostAllSet(weights,bias,weightsList,biasList,currentCost,currentGradientNorm,costs,gradientNorms,X,Y,L,P,nbNeurons,globalIndices,activations,epsClose,nbDichotomie,eRelative);
    }
    else
    {
