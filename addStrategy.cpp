...
std::vector<std::string> const& activations, Sdouble const& epsClose, int const& nbDichotomie, Sdouble const& eRelative)
{
    ...
    while (i<taille && continuer)
    {
        ...
        if (dis-std::abs(dis.error)>epsClose)
        {
            if (dis+std::abs(dis.error)<disMin-std::abs(disMin.error)){disMin=dis; indiceCloser=i;}
            i++;
        }
        else{continuer=false;}
    }
    ...
    if(i==taille)
    {
        ...
        if(errorRelative-std::abs(errorRelative.error)>eRelative)
        {
            weightsList.push_back(weights); biasList.push_back(bias);
            costs.push_back(currentCost); gradientNorms.push_back(currentGradientNorm);
            return true;
        ...
        if (gradientNormLeft+std::abs(gradientNormLeft.error)<gradientNormRight-std::abs(gradientNormRight.error))
        {
        ...
        for(j=0;j<nbDichotomie;j++)
        {
            ...
            if (gradientNormInter+std::abs(gradientNormInter.error)<gradientNormMin-std::abs(gradientNormMin.error))
            {
    ...
    if(i==taille)
    {
        while(k<taille && !plat)
        {
            ...
            while(j<nbDichotomie)
            {
                ...
                if(costLeft+std::abs(costLeft.error)<costRight-std::abs(costRight.error))
                {
                    std::copy(weightsInter.begin(),weightsInter.end(),weightsRight.begin()); std::copy(biasInter.begin(),biasInter.end(),biasRight.begin());
                }
                else
...
std::vector<std::string> const& activations, Sdouble const& epsClose, int const& nbDichotomie, Sdouble const& eRelative)
{
    ...
    else
    {
        ...
        while (i<taille && continuer)
        {
            dis = Sstd::abs(currentCost-costs[i]);
            if (dis-std::abs(dis.error)>epsClose)
            {
                i++;
            }
