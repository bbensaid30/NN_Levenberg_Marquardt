...
bool const tracking, bool const record, std::string const fileExtension)
{
    ...
    std::ofstream speedFlux(("Record/speed_SGD_"+fileExtension+".csv").c_str());
...
bool const tracking, bool const track_continuous, bool const record, std::string const fileExtension)
{
    ...
    std::ofstream speedFlux(("Record/speed_Momentum_Euler_"+fileExtension+".csv").c_str());
    if(!weightsFlux){std::cout << "Impossible d'ouvrir le fichier" << std::endl;}
...
std::map<std::string,Sdouble> Momentum_bias(Eigen::SMatrixXd& X, Eigen::SMatrixXd& Y, int const& L, std::vector<int> const& nbNeurons, std::vector<int> const& globalIndices,
std::vector<std::string> const& activations, std::vector<Eigen::SMatrixXd>& weights, std::vector<Eigen::SVectorXd>& bias, std::string const& type_perte, Sdouble const& learning_rate,
int const& batch_size, Sdouble const& beta1, Sdouble const& eps, int const& maxIter,
bool const tracking, bool const track_continuous, bool const record, std::string const fileExtension)
{

    std::ofstream weightsFlux(("Record/weights_Momentum_bias_"+fileExtension+".csv").c_str());
    std::ofstream speedFlux(("Record/speed_Momentum_bias_"+fileExtension+".csv").c_str());
    if(!weightsFlux){std::cout << "Impossible d'ouvrir le fichier" << std::endl;}
    ...
    std::vector<Eigen::SMatrixXd> slopes(L);
    Eigen::SVectorXd gradient = Eigen::SVectorXd::Zero(N);
    Eigen::SVectorXd moment1 = Eigen::SVectorXd::Zero(N), moment1N(N);
    ...
    //pour le batch size
        ...
        for(batch=0; batch<number_batch;batch++)
        {
            ...
            moment1 = (1-beta1)*moment1 + beta1*gradient;
            moment1N = moment1/(1-Sstd::pow(1-beta1,iter+1));
            if(track_continuous)
            {
                condition = moment1.dot(gradient);
                if(condition>=0){continuous_entropie++;}
            ...
            update(L,nbNeurons,globalIndices,weights,bias,-learning_rate*moment1N);
...
std::map<std::string,Sdouble> Adam_bias(Eigen::SMatrixXd& X, Eigen::SMatrixXd& Y, int const& L, std::vector<int> const& nbNeurons, std::vector<int> const& globalIndices,
std::vector<std::string> const& activations, std::vector<Eigen::SMatrixXd>& weights, std::vector<Eigen::SVectorXd>& bias, std::string const& type_perte, Sdouble const& learning_rate,
int const& batch_size, Sdouble const& beta1, Sdouble const& beta2, Sdouble const& eps, int const& maxIter,
bool const tracking, bool const record, std::string const fileExtension)
{
    ...
    Eigen::SVectorXd gradient = Eigen::SVectorXd::Zero(N);
    Eigen::SVectorXd moment1 = Eigen::SVectorXd::Zero(N), moment1N(N);
    Eigen::SArrayXd moment2 = Eigen::SArrayXd::Zero(N), moment2N(N);
    ...
    while (moyGradientNorm+std::abs(moyGradientNorm.error)>eps && iter<maxIter)
    {
        ...
        for(batch=0; batch<number_batch;batch++)
        {
            ...
            moment2 = (1-beta2)*moment2.array() + beta2*gradient.array().pow(2);
            moment1N = moment1/(1-Sstd::pow(1-beta1,iter+1));
            moment2N = moment2/(1-Sstd::pow(1-beta2,iter+1));
            update(L,nbNeurons,globalIndices,weights,bias,-learning_rate*moment1N.array()*((moment2N+std::pow(10,-10)).rsqrt()));
            fforward(L,number_data,nbNeurons,activations,weights,bias,As,slopes);
            backward(echantillonY,L,number_data,nbNeurons,activations,globalIndices,weights,bias,As,slopes,gradient,type_perte);
...
std::map<std::string,Sdouble> AMSGrad_bias(Eigen::SMatrixXd& X, Eigen::SMatrixXd& Y, int const& L, std::vector<int> const& nbNeurons, std::vector<int> const& globalIndices,
std::vector<std::string> const& activations, std::vector<Eigen::SMatrixXd>& weights, std::vector<Eigen::SVectorXd>& bias, std::string const& type_perte, Sdouble const& learning_rate,
int const& batch_size, Sdouble const& beta1, Sdouble const& beta2, Sdouble const& eps, int const& maxIter, bool const record, std::string const fileExtension)
{

    std::ofstream weightsFlux(("Record/weights_AMSGrad_bias_"+fileExtension+".csv").c_str());
    if(!weightsFlux){std::cout << "Impossible d'ouvrir le fichier" << std::endl;}
    ...
    while (moyGradientNorm+std::abs(moyGradientNorm.error)>eps && iter<maxIter)
    {
        ...
        for(batch=0; batch<number_batch;batch++)
        {
            ...
            moment1N = moment1/(1-Sstd::pow(1-beta1,iter+1));
            moment2N = moment2/(1-Sstd::pow(1-beta2,iter+1));
            if(iter==0){max_moment2N = moment2N;}
            else{max_moment2N = max_moment2N.max(moment2N);}
            update(L,nbNeurons,globalIndices,weights,bias,-learning_rate*moment1N.array()*((max_moment2N+std::pow(10,-10)).rsqrt()));
            fforward(L,number_data,nbNeurons,activations,weights,bias,As,slopes);
            backward(echantillonY,L,number_data,nbNeurons,activations,globalIndices,weights,bias,As,slopes,gradient,type_perte);
            sommeGradient += gradient.norm();
...
bool const tracking, bool const track_continuous, bool const record, std::string const fileExtension)
{
    ...
    else if(algo=="Momentum_bias")
    {
        study = Momentum_bias(X,Y,L,nbNeurons,globalIndices,activations,weights,bias,type_perte,learning_rate,batch_size,beta1,eps,maxIter,tracking,track_continuous,record,fileExtension);
    }
    ...
    else if(algo=="Adam_bias")
    {
        study = Adam_bias(X,Y,L,nbNeurons,globalIndices,activations,weights,bias,type_perte,learning_rate,batch_size,beta1,beta2,eps,maxIter,tracking,record,fileExtension);
    }
    ...
    else if(algo=="AMSGrad_bias")
    {
        study = AMSGrad_bias(X,Y,L,nbNeurons,globalIndices,activations,weights,bias,type_perte,learning_rate,batch_size,beta1,beta2,eps,maxIter,record,fileExtension);
    }
    else
    {
